...
export function initFilters() {
  ...
  fetch('/data/data.json')
    ...
    .then((data) => {
      ...
      const isLandPage = document.querySelector('.filter__title')?.textContent.includes('POZEMKOV');
...
export function renderFilters(data) {
  ...
  createSelectFilter(
    'Typ',
    'type',
    [{ value: 'all', label: 'Všetky' }, ...uniqueArray(data.map((d) => d.Typ.toLowerCase()))].map((t) => ({
      value: t.value || t,
  ...
  createSelectFilter(
    'Dostupnosť',
    'status',
    [{ value: 'all', label: 'Všetky' }, ...uniqueArray(data.map((d) => d.Stav.toLowerCase()))].map((s) => ({
      value: s.value || s,
...
export function renderLandFilters(data) {
  ...
  if (!selectsContainer || !rangesContainer || !listBody) {
    console.warn('[land filters] Missing filter containers on this page. Skipping render.');
    return;
  }
  ...
  function renderList() {
    let filtered = landData.filter((item) => {
      if (
        ...
        return false;
      if (item.Plocha_spolu < filters.plocha[0] || item.Plocha_spolu > filters.plocha[1])
        return false;
      const price = item.Cena_s_DPH || 0;
      if (price < filters.cena[0] || price > filters.cena[1]) return false;
      return true;
    ...
    if (filtered.length === 0) {
      ...
      if (totalCountBottom) totalCountBottom.textContent = String(landData.length);
      if (showedCount) showedCount.textContent = '0';
      if (loadMoreBtn) loadMoreBtn.style.display = 'none';
      return;
    }
    ...
    listBody.innerHTML = filtered
      ...
      .map((item) => {
        ...
        <a href="floor.html" target="_blank" class="${rowClass}">
          ...
          <div class="filter__list-col">${item.Cena_s_DPH ? item.Cena_s_DPH.toLocaleString() + ' €' : '-'}</div>
          <div class="filter__list-col">${item.Stav}</div>
        </a>
      `;
      })
  ...
  createSelectFilter(
    'Dostupnosť',
    'status',
    [{ value: 'all', label: 'Všetky' }, ...uniqueArray(landData.map((d) => d.Stav.toLowerCase()))].map((s) => ({
      value: s.value || s,
      label: s.label || s.charAt(0).toUpperCase() + s.slice(1),
    })),
  );
